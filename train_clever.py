...
from t5_pretrainer.mixlora import CLEVER as MixLoraDSI
from t5_pretrainer.mixlora_config import MixLoraConfig
from t5_pretrainer.mixlora_trainer import MixLoraDSI_CLEVER_Trainer
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="nq320k")
    ...
    # Data collator
    data_collator = RiporForSeq2seqCollator(tokenizer_type="t5-base", max_length=256) # 512 This is inconsistent with MixLoRADSI, which is set to 256, might need to rerun
    ...
    simple_dataloader = torch.utils.data.DataLoader(
        RiporForSeq2seqDataset(
            example_path=f"./mixloradsi/{args.dataset}/d{args.taskid-1}/doc2query/query_to_docid.train.json",
            docid_to_smtid_path=f"./mixloradsi/{args.dataset}/d{args.taskid-1}/experiments/rq_smtid/docid_to_tokenids.json",
        ),
        batch_size=512,
        shuffle=True,
    ...
    # if args.taskid == 1:
    #     model._before_task(dataloader=simple_dataloader, task_id=args.taskid)
    # else:
    model.ewc.num_samples = 512  # 1024 512
    if args.taskid > 2:
        exp_name = "_".join(args.run_name.split("_")[1:])
        model.ewc = torch.load(
            os.path.join(f"./mixloradsi/{args.dataset}/d{args.taskid - 1}/experiments/{exp_name}", "ewc.pt")
        )
    model._before_task(dataloader=simple_dataloader, task_id=args.taskid)

    os.makedirs(args.output_dir, exist_ok=True)

    torch.save(model.ewc, os.path.join(args.output_dir, "ewc.pt"))

    trainer = MixLoraDSI_CLEVER_Trainer(
        model=model,
        args=training_args,
