...
data_root_dir=./mixloradsi/nq320k/d0
collection_path=$data_root_dir/full_collection/
queries_path=$data_root_dir/train_queries

experiment_dir=experiments
output_dir=./mixloradsi/nq320k/d0/$experiment_dir
...
teacher_score_path=$data_root_dir/bm25_top100/hacked_qrel_added_qid_docids_teacher_scores.train.json
...
# 2.1 Normal dense indexing using M0 -> Results in a lot of embeddings file
...
torchrun --nproc-per-node=1 rerank.py \
    --run_json_path=$out_dir/nq320k_train/run.json \
    --out_dir=$out_dir/nq320k_train \
    --collection_path=$collection_path \
...
python3 t5_pretrainer/add_qrel_to_rerank_run.py \
    --teacher_score_path=$data_root_dir/train_queries/hacked_qid_to_reldocid_to_score.json \
    --rerank_path=$out_dir/nq320k_train/qid_docids_teacher_scores.train.json \
