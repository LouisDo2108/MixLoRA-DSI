...
initial_mixlora_config_path=./MixLoraDSI/config/longeval/mixloradsi-pt_10%.json
num_tasks=3
...
do
  ...
  if [ $i -eq 1 ]; then
    ...
    mixlora_config_json_path=./mixloradsi/longeval/$previous_split/$experiment_dir/$run_name/mixlora_config.json
    epoch=3
  fi
  ...
  do
        ...
        mixlora_config_json_path=./mixloradsi/longeval/$train_split/$experiment_dir/$run_name/mixlora_config.json

        torchrun --master_port=25900 --nproc_per_node=1 eval_mixlora.py \
                --pretrained_path=$model_dir \
                --mixlora_config_json_path=$mixlora_config_json_path \
